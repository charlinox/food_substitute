...
class ProductRepository(Repository):
    ...
    def add_product(self, product, store):
        self.db.query("""
            INSERT IGNORE INTO Asso_product_store(product_id, store_id)
            VALUES (:product_id, :store_id)
        """, product_id=product.id, store_id=store.id)
...
class CategoryRepository(Repository):
    ...
    def get_all_by_category(self, product):
        products = self.db.query(f"""
            ...
            JOIN product ON Asso_product_category.category_id = category.id
            WHERE product.id = :id
        """, id=product.id).all(as_dict=True)
        return [self.model(**product) for product in products]
...
class FavoriteRepository(Repository):
    ...
    table = 'favorite' # Plutot l'initiale en majuscule non d'ailleurs ? #
