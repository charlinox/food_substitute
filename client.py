...
from files.constants import CATEGORY_LIST
...
class Client:
    ...
    def main_loop(self):
        ...
        while True:
            ...
            main_choice = input(
                "  Sélectionnez votre choix en indiquant son numéro : ")
    ...
    def product_loop(self, category_choice):
        ...
        while True:
            ...
            products_by_category = category_by_choice.all_products
            list_product = []
            print("\n")
            for i, product in enumerate(products_by_category):
                list_product.append(product)
                if i < 10:
                    print(f"  {i+1} - {product.name} - Valeur nutritionnelle :"
                          f" {product.nutrition_grade.upper()}")
                else:
                    break
            product_choice = input("\n  Sélectionnez l'aliment en entrant son"
                                   " numéro parmis les choix suivants : ")
    ...
    def substitutes_loop(self, product_choice):
        ...
        while True:
            ...
            for i, substitute in enumerate(best_substitutes):
                if i < 10:
                    print(f"  {i+1} - {substitute.name} - Valeur"
                          f" nutritionnelle : "
                          f"{substitute.nutrition_grade.upper()}")
                    list_substitutes.append(substitute)
                else:
                    break
            substitute_choice = input("\n  Sélectionnez un substitut en "
                                      "entrant son numéro parmis les choix"
                                      " suivants : ")
    ...
    def substitut_display(self, substitute_choice, product_choice):
        ...
        while True:
            print(
                f"\n"
                f"  Voici les détails du substitut que vous avez "
                f"sélectionné :\n"
                f"  Nom du produit : {substitute_choice.name}\n"
                f"  Au moins un magasin ou l'acheter :"
            )
            stores = Store.objects.get_some_by_product(substitute_choice, 3)
            for store in stores:
                print(f"    {store.name}")
            print(
                f"  Score nutritionnel : "
                f"{substitute_choice.nutrition_grade.upper()}\n"
    ...
    def favorite_display(self):
        """  Menu displaying the details of the original
        and substitued products  """
        favorites = Favorite.objects.get_all_favorite()
        for i, favorite in enumerate(favorites):
            print(
                "\n"
                f"Le produit original     ==>  "
                f"{favorite.product_as_original}\n"
                f"   Le produit substitué ==>  "
                f"{favorite.product_as_substitut}\n"
                f"   url                     : "
                f"{favorite.url}\n"
                f"   nutrition_grade         : "
                f"{favorite.nutrition_grade.upper()}\n"
                f"   Magasins                : "
                f"{favorite.stores}\n"
                )
            if i % 3 == 0 and i != 0:
                print("\n  Veuillez entrer 'O' pour afficher les substituts "
                      "suivants ou une autre touche pour revenir au "
                      "menu principal : ")
                again_choice = input("\n").upper()
                if again_choice == "O":
                    continue
    ...
    def start(self):
        ...
        if main_choice == "1":
            ...
            self.substitut_display(substitute_choice, product_choice)
        if main_choice == "2":
